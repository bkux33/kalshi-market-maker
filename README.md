...
# Real data (Kalshi API key required for the WebSocket)
cp .env.example .env                               # set KALSHI_API_KEY_ID, KALSHI_PRIVATE_KEY_PATH
alphalab record --series KXBTC15M KXETH15M --external
alphalab ingest && alphalab research-loop && alphalab scorecard
```
...
| `KALSHI_ENV` | no (default `demo`) | `demo` or `prod` |
